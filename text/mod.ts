...
export function didYouMean(
  ...
  if (!possibleWords.includes(guessWord) && autoThrow) {
    ...
    if (suggestionLimit == 1 && suggestions.length > 0) {
      throw new Error(
        `For ${JSON.stringify(guessWord)}, did you mean ${
          JSON.stringify(suggestions[0])
        }?`,
      );
    } else {
      throw new Error(
        `For ${JSON.stringify(guessWord)}, did you mean one of ${
  ...
  return [...possibleWords].sort((a, b) =>
    levenshteinDistance(guessWord, a) - levenshteinDistance(guessWord, b)

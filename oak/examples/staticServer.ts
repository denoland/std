...
/*
 * This is a basic example of a static server which serves the content
 * of `./static`.  It assumes you are running the server from the root
 * of `std`
 */

import { green, cyan, bold, yellow, red } from "../../colors/mod.ts";
...
(async () => {
  ...
  app.use(async context => {
    await send(context, context.request.path, {
      root: `${deno.cwd()}/oak/examples/static`,
      index: "index.html"
    });
  });

...
Deno.test("spawn", async (t) => {
  ...
  await t.step(
    "the 'error' event is emitted when no binary is found",
    async () => {
  ...
  await t.step(
    "the 'exit' event is emitted with an exit code after the child process ends",
    async () => {
  ...
  await t.step({
    name: "verify that stdin and stdout work",
    fn: async () => {
      const promise = withTimeout(3000);
  ...
  await t.step({
    name: "stdin and stdout with binary data",
    fn: async () => {
  ...
  await t.step({
    ignore: isWindows,
    name: "verify that environment values can be numbers",
    async fn() {
  ...
  await t.step({
    ignore: isWindows,
    name: "verify that environment values can be booleans",
    async fn() {
  ...
  await t.step("'spawn' event", async () => {
  ...
  await t.step("Verify that a shell is executed", async () => {
    const promise = withTimeout(3000);
  ...
  await t.step({
    ignore: isWindows,
    name: "verify that passing arguments works",
    async fn() {
      const promise = withTimeout(3000);
  ...
  await t.step({
    ignore: isWindows,
    name: "verity that shell features can be used",
    async fn() {
      const promise = withTimeout(3000);
  ...
  await t.step({
    ignore: isWindows,
    name: "verity that environment is properly inherited",
    async fn() {
      const promise = withTimeout(3000);
      const env = spawn(
        `"${Deno.execPath()}" eval -p "Deno.env.toObject().BAZ"`,
...
Deno.test("execFile", async (t) => {
  await t.step({
    name: "get stdout as a string",
    async fn() {
      let child: unknown;
  ...
  await t.step({
    name: "exceed given maxBuffer limit",
    async fn() {
      let child: unknown;
      const script = path.join(
        path.dirname(path.fromFileUrl(import.meta.url)),

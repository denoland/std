...
Deno.test({
  ...
  fn(): void {
    assert(match("*", "foo", { globstar: false }));
    assert(match("*", "foo", { globstar: false }));
    assert(match("f*", "foo", { globstar: false }));
    assert(match("f*", "foo", { globstar: false }));
    assert(match("*o", "foo", { globstar: false }));
    assert(match("*o", "foo", { globstar: false }));
    assert(match("u*orn", "unicorn", { globstar: false }));
    assert(match("u*orn", "unicorn", { globstar: false }));
    assert(!match("ico", "unicorn", { globstar: false }));
    assert(match("u*nicorn", "unicorn", { globstar: false }));
    assert(match("u*nicorn", "unicorn", { globstar: false }));
  },
});
...
Deno.test({
  ...
  fn(): void {
    assert(match("f?o", "foo", { globstar: false }));
    assert(match("f?o?", "fooo", { globstar: false }));
    assert(!match("f?oo", "foo", { globstar: false }));
    assert(!match("?fo", "fooo", { globstar: false }));
    assert(!match("f?oo", "foo", { globstar: false }));
    assert(!match("foo?", "foo", { globstar: false }));
  },
});
...
Deno.test({
  ...
  fn(): void {
    assert(match("fo[oz]", "foo", { globstar: false }));
    assert(match("fo[oz]", "foz", { globstar: false }));
    assert(!match("fo[oz]", "fog", { globstar: false }));
    assert(match("fo[a-z]", "fob", { globstar: false }));
    assert(!match("fo[a-d]", "fot", { globstar: false }));
    assert(!match("fo[!tz]", "fot", { globstar: false }));
    assert(match("fo[!tz]", "fob", { globstar: false }));
  },
});
...
Deno.test({
  ...
  fn(): void {
    ...
    for (const c of "09AGZagz") {
      assert(match("[[:alnum:]]", c, { globstar: false }), c);
    }
    for (const c of "AGZagz") {
      assert(match("[[:alpha:]]", c, { globstar: false }), c);
    }
    for (const c of "\x00\x20\x7F") {
      assert(match("[[:ascii:]]", c, { globstar: false }), c);
    }
    for (const c of "\t ") {
      assert(match("[[:blank:]]", c, { globstar: false }), c);
    }
    for (const c of "\x00\x1F\x7F") {
      assert(match("[[:cntrl:]]", c, { globstar: false }), c);
    }
    for (const c of "09") {
      assert(match("[[:digit:]]", c, { globstar: false }), c);
    }
    for (const c of "\x21\x7E") {
      assert(match("[[:graph:]]", c, { globstar: false }), c);
    }
    for (const c of "az") {
      assert(match("[[:lower:]]", c, { globstar: false }), c);
    }
    for (const c of "\x20\x7E") {
      assert(match("[[:print:]]", c, { globstar: false }), c);
    }
    for (const c of "!\"#$%&'()*+,-./:;<=>?@[\\]^_‘{|}~") {
      assert(match("[[:punct:]]", c, { globstar: false }), c);
    }
    for (const c of "\t\n\v\f\r ") {
      assert(match("[[:space:]]", c, { globstar: false }), c);
    }
    for (const c of "AZ") {
...
Deno.test({
  ...
  fn(): void {
    assert(
      match("foo{bar,baaz}", "foobaaz", { globstar: false }),
    );
    assert(
      match("foo{bar,baaz}", "foobar", { globstar: false }),
    );
    assert(
      !match("foo{bar,baaz}", "foobuzz", { globstar: false }),
    );
    assert(
      match("foo{bar,b*z}", "foobuzz", { globstar: false }),
    );
  },
});
...
Deno.test({
  ...
  fn(): void {
    ...
    assert(!match("?(foo).txt", "foo.txt"));
    assert(match("?(foo).txt", "a(foo).txt"));
...
Deno.test({
  ...
  fn(): void {
    assert(match("*(foo).txt", "foo.txt", { extended: true }));
    assert(!match("*(foo).txt", "foo.txt"));
    assert(match("*(foo).txt", "bar(foo).txt"));
...
Deno.test({
  ...
  fn() {
    assert(match("{foo,bar}/[ab", "foo/[ab", { extended: true }));
    assert(match("{foo,bar}/{foo,bar", "foo/{foo,bar", { extended: true }));
    assert(match("{foo,bar}/?(foo|bar", "foo/?(foo|bar", { extended: true }));
    assert(match("{foo,bar}/@(foo|bar", "foo/@(foo|bar", { extended: true }));
    assert(match("{foo,bar}/*(foo|bar", "foo/*(foo|bar", { extended: true }));
    assert(match("{foo,bar}/+(foo|bar", "foo/+(foo|bar", { extended: true }));
    assert(match("{foo,bar}/!(foo|bar", "foo/!(foo|bar", { extended: true }));
    assert(match("{foo,bar}/?({)}", "foo/?({)}", { extended: true }));
    assert(match("{foo,bar}/{?(})", "foo/{?(})", { extended: true }));
  },
});
...
Deno.test({
  ...
  fn() {
    const pattern1 = globToRegExp("?(foo|bar)", { extended: true });

...
import { rangeSetIntersects } from "./range_set_intersects.ts";
  ...
  fn: async (t) => {
    ...
    for (const [r1, r2, expected] of versions) {
      await t.step({
        name: `${r1} ∩ ${r2}`,
        fn: () => {
          const range1 = parseRangeSet(r1);
          const range2 = parseRangeSet(r2);
          const actual1 = rangeSetIntersects(range1, range2);
          const actual2 = rangeSetIntersects(range2, range1);

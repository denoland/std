...
Deno.test("SeededRandom constructor validates its parameters", async (t) => {
  await t.step("seed length ok", () => {
    new SeededRandom(new Uint8Array(16));
  });

  await t.step("seed too long", () => {
    assertThrows(() => new SeededRandom(new Uint8Array(17)));
  });

  await t.step("seed too short", () => {
    assertThrows(() => new SeededRandom(new Uint8Array(15)));
  });
});
...
Deno.test("SeededRandom's `seed` parameter is converted to state", () => {
  const expectedState = {
    algorithm: "pcg32",
    state: new Uint8Array([0xc9, 0x51, 0xcc, 0x10, 0xc1, 0xcc, 0x0d, 0xc5]),
    inc: new Uint8Array([0x37, 0x26, 0xf9, 0x47, 0xb4, 0x00, 0x7f, 0xfb]),
  } as const;

  // deno-fmt-ignore
  const prng1 = new SeededRandom(new Uint8Array([

...
import { decodeBase64, encodeBase64 } from "./unstable_base64.ts";
...
Deno.test("encodeBase64() encodes binary", () => {
  for (const [input, output] of testsetBinary) {
    assertEquals(encodeBase64(input.slice()), output);
  }
});

Deno.test("encodeBase64() encodes binary buffer", () => {
  for (const [input, output] of testsetBinary) {
    assertEquals(encodeBase64(input.buffer.slice() as ArrayBuffer), output);
  }
});

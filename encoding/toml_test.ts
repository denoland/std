...
Deno.test({
  name: "Stringify with string values",
  fn: (): void => {
    const src = {
      '"': '"',
      "'": "'",
...
Deno.test({
  name: "Inline Array of Inline Table",
  fn(): void {
    const expected = {
      inlineArray: {
        string: [{ var: "a string" }],
...
Deno.test({
  name: "Parse malformed local time as String (#8433)",
  fn(): void {
...
Deno.test({
  name: "Single-line string comment error",
  fn(): void {
    assertThrows(
      (): void => {
...
Deno.test({
  name: "stringfy with key alignment",
  fn(): void {
    const src = {
      "a": 1,
      "aa": 1,

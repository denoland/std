...
Deno.test("digest", async (t) => {
  await t.step(
    "different ways to perform the same operation should produce the same result",
    async () => {
      ...
      assertEquals(
        toHexString(
          await stdCrypto.subtle.digest("SHA-384", new ArrayBuffer(0)),
        ),
        emptyDigest,
      );
    },
  ...
  await t.step("should return an ArrayBuffer", async () => {
    const inputString = "taking the hobbits to isengard";
    const inputBytes = new TextEncoder().encode(inputString);
  ...
  await t.step("should not ignore length option", async () => {
    const inputString = "taking the hobbits to isengard";
    const inputBytes = new TextEncoder().encode(inputString);
  ...
  await t.step(
    "memory use should remain reasonable even with large inputs",
    async () => {
  ...
  await t.step(
    "memory use should remain reasonable even with many calls",
    async () => {
      const process = Deno.spawnChild(Deno.execPath(), {
        args: ["--quiet", "run", "--no-check", "-"],
        cwd: moduleDir,
  ...
  const digestCases: [
    ...
    [
      ...
      {},
      {
        ...
        "KECCAK-224":
          "93ef86f74392e3a635c65df746b0f990bcb13b053301e52f51fab144",
        "KECCAK-256":
    ...
    [
      ...
      {},
      {
        ...
        "KECCAK-224":
          "5b620623f63faf310190cd69fada226f4ba51b5766c2eef4d69ec7f8",
        "KECCAK-256":
  ...
  await t.step("fnv algorithm implementation", () => {
  ...
  for (const algorithm of digestAlgorithms) {
    await t.step(`${algorithm} test vectors`, async () => {
      for (

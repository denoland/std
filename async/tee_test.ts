...
Deno.test("tee() checks 2 branches", async () => {
  const iter = gen();
...
Deno.test("tee() checks 3 branches with immediate consumption", async () => {
  const iter = gen();
...
Deno.test("tee() checks 3 branches with delayed consumption", async () => {
  const iter = gen();
...
Deno.test("tee() checks concurrent next() calls", async () => {
  const [left] = tee(gen());

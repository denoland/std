...
Deno.test("called", async function () {
  let called = 0;
...
Deno.test("canceled", async function () {
  let called = 0;
  const d = debounce(() => called++, 100);
  d();
  d();
...
Deno.test("with params & context", async function () {
...
Deno.test("with types", async function () {

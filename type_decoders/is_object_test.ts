...
test({
  name:
    "isObject({stringDecoder, numberDecoder}, {noExcessProperties: true, allErrors: true})",
  fn: () => {
    const decoder = isObject(
      {
        string: stringDecoder,
...
test({
  name:
    "isObject({stringDecoder, isObject({numberDecoder})}, {noExcessProperties: true, allErrors: true})",
  fn: () => {
    const decoder = isObject(
      {
        string: stringDecoder,
    ...
    assertDecodesToErrors(decoder, obj3, [
      ...
      new DecoderError(
        obj3,
        'invalid value for key ["object"] > invalid value for key ["number"] > must be a number',
        {
          decoderName: "isObject",
...
test({
  name:
    "async isObject({stringDecoder, isObject({numberDecoder})}, {noExcessProperties: true, allErrors: true})",
  fn: async () => {
    const decoder = isObject(
      {
        string: stringDecoder,

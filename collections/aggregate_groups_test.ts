...
Deno.test({
  name: "aggregateGroups() checks no mutation",
  fn() {
    const input = {
      Woody: [2, 3, 1, 4],
      Buzz: [5, 9],
...
Deno.test({
  name: "aggregateGroups() checks string building using all params",
  fn() {
    aggregateGroupsTest(
      [
        {
...
Deno.test({
  name: "aggregateGroups() checks sum ignoring non reduce params",
  fn() {
    aggregateGroupsTest(
      [
        {
...
Deno.test({
  name: "aggregateGroups() checks empty input",
  fn() {
    aggregateGroupsTest([
      {},
      () => 1,

...
Deno.test("[collections/takeFirstWhile] Num array", () => {
    let arr = [1, 2, 3, 4, 5, 6];
    let actual = takeFirstWhile(arr, (i) => i !== 4);
...
Deno.test("[collections/takeFirstWhile] Add two to each num in predicate", () => {
    let arr = [1, 2, 3, 4, 5, 6];
    let actual = takeFirstWhile(arr, (i) => i + 2 !== 6);
...
Deno.test("[collections/sumOf] NaN and Infinity", () => {
    ...
    assertEquals(actual3, [1, 2, Number.POSITIVE_INFINITY, 3, 4, Number.NEGATIVE_INFINITY, 5, 6])
});

...
Deno.test("Empty input returns false", () => {
  const input = {};
...
Deno.test("Returns false when it doesn't include the value", () => {
  const input = {
    first: 33,
    second: 34,
  };
...
Deno.test("Non-primitive values", () => {
  const input = {
    first: {},
  };
...
Deno.test("Same behaviour as naive impl", () => {
  const input = {
    first: 42,
  };
...
Deno.test("prevent enumerable prototype check", () => {
  class Foo {}
  // @ts-ignore: for test

...
test({name: 'globrex: Standard * matching', fn() {
...
test({name: 'globrex: advance * matching', fn() {
...
test({name: 'globrex: ? match one character, no more and no less', fn() {
...
test({name: 'globrex: [] match a character range', fn() {
...
test({name: 'globrex: [] extended character ranges', fn() {
...
test({name: 'globrex: {} match a choice of different substrings', fn() {
...
test({name: 'globrex: complex extended matches', fn() {
...
test({name: 'globrex: standard globstar', fn() {
...
test({name: 'globrex: remaining chars should match themself', fn() {
...
test({name: 'globrex: globstar advance testing', fn() {
...
test({name: 'globrex: extended extglob ?', fn() {
...
test({name: 'globrex: filepath path-regex', fn() {
   let opts = { extended:true, filepath:true, globstar: false }, res, pattern;
...
test({name: 'globrex: filepath path segments', fn() {
...
test({name: 'globrex: stress testing', fn() {

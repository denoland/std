...
export function buildDefaultLogMessage<L extends LogLevels, M, A>(logLevel: keyof L, message: M, additionalData: A) {
    return `[${logLevel}] ${new Date().toLocaleDateString()} ${asString(message)}${additionalData ? ` ${asString(additionalData)}` : ''}}`
}
...
export function buildLogger<L extends LogLevels, M, A>(
    ...
    return mapEntries(logLevels, ([level]) => [
        level,
        (message: M, additionalData: A) => dispatch(
            logLevels,
            level,
            thresholdLevel,
            message,
